...
# 오네시스 기본 디자인 스타일 — 특별한 지시가 없으면 모든 디자인은 이걸 따른다.
_DESIGN_STYLE = (
    "[오네시스 기본 디자인 스타일 — 사용자가 다른 스타일을 명시하지 않는 한 항상 이대로]\n"
    "- 인스타그램처럼 심플·미니멀. 흰색 또는 아주 옅은 회색 배경, 넉넉한 여백.\n"
    "- 색은 '단색(플랫)'. 그라데이션은 쓰지 않는다(정말 필요하면 딱 1곳). 포인트 컬러는 단색 1개.\n"
    "- 아이콘은 이모지(❤️💬 등) 금지, '선(line)으로 된 얇은 아이콘'만 사용.\n"
    "- 얇은 테두리(1px 옅은 회색), 부드러운 그림자, 둥근 모서리(14~20px).\n"
    "- 타이포 위계는 뚜렷하게, 시스템 폰트. 요소는 정돈되고 간결하게."
)
...
_DEEP_BUILD_FORMAT = (
    ...
    "**[분량 배분 — 매우 중요]** 위 7개 섹션을 '전부' 포함하세요. 앞 섹션(개요·스택·페이지)에 "
    "분량을 너무 쏟아 뒤 섹션(특히 5.기능별 구현, 6.데이터 모델, 7.완료 기준)이 빠지면 안 됩니다. "
    "각 섹션을 균형 있게, 페이지·기능이 많으면 핵심부터 간결히 다뤄서라도 7개 섹션을 끝까지 완성하세요.\n\n"
    + _DESIGN_STYLE
)
...
def mockup_system() -> str:
    return (
        ...
        "- 색은 '단색(플랫)'을 기본으로 한다. **그라데이션은 거의 쓰지 말 것**(꼭 필요하면 딱 1곳만). "
        "흰색/아주 옅은 회색 바탕 + 단색 포인트 컬러 1개. 부드러운 그림자, 둥근 모서리(14~20px).\n"
        ...
        "- **아이콘은 이모지(❤️💬🔍🏠 등)를 절대 쓰지 말 것.** 인스타그램처럼 '선(line)으로 된 "
        "인라인 SVG 아이콘'을 직접 그려서 사용한다. 규격: <svg viewBox=\"0 0 24 24\" fill=\"none\" "
        "stroke=\"currentColor\" stroke-width=\"1.8\" stroke-linecap=\"round\" stroke-linejoin=\"round\">. "
        "하트·댓글·공유·저장(북마크)·홈·검색·프로필·설정 등 모든 UI 아이콘을 이렇게 SVG로 그린다.\n"
        "- 사진/썸네일 자리는 '옅은 단색 블록'으로 대체(그라데이션 말고 단색).\n"
        "- 모바일 세로(가로 ~390px) 우선.\n"
